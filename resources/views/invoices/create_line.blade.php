...
<div class="max-w-3xl mx-auto bg-white shadow p-6 rounded">
  <h1 class="text-2xl font-light mb-4">Add Line to Invoice: {{ $invoice->invoice_number }}</h1>
  ...
  <form action="{{ route('invoices.store_line', $invoice->id) }}" method="POST" class="space-y-4">
    @csrf
    <div>
    ...
    <div class="flex space-x-4">
      ...
      <a href="{{ route('invoices.show', $invoice->id) }}" class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700">Cancel</a>
    </div>
  </form>
</div>
@endsection
