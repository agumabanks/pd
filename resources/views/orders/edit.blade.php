...
<div class="max-w-3xl mx-auto bg-white shadow p-6 rounded">
  <h1 class="text-2xl font-light mb-4">Edit Order</h1>
  ...
  <form action="{{ route('orders.update', $order->id) }}" method="POST" class="space-y-4">
    ...
    <div>
      ...
      <input type="text" name="change_order_status" id="change_order_status" value="{{ old('change_order_status', $order->change_order_status) }}"
    ...
    <div>
      ...
      <input type="date" name="requested_delivery_date" id="requested_delivery_date" value="{{ old('requested_delivery_date', $order->requested_delivery_date) }}"
             class="mt-1 block w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-600">
    </div>
    <div>
      <label for="promised_delivery_date" class="block text-gray-700">Promised Delivery Date</label>
      <input type="date" name="promised_delivery_date" id="promised_delivery_date" value="{{ old('promised_delivery_date', $order->promised_delivery_date) }}"
             class="mt-1 block w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-600">
    </div>
    <div>
      <label for="acknowledge_due_date" class="block text-gray-700">Acknowledge Due Date</label>
      <input type="date" name="acknowledge_due_date" id="acknowledge_due_date" value="{{ old('acknowledge_due_date', $order->acknowledge_due_date) }}"
             class="mt-1 block w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-600">
    </div>
    <div>
      <label for="pdf_url" class="block text-gray-700">PO PDF URL</label>
      <input type="url" name="pdf_url" id="pdf_url" value="{{ old('pdf_url', $order->pdf_url) }}"
             class="mt-1 block w-full border border-gray-300 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-600">
    </div>
