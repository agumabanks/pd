                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none" x-show="errors.country">
                                    <svg class="h-5 w-5 text-red-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                                    </svg>
                                </div>
                            </div>
                            <p class="mt-1 text-sm text-red-600" x-show="errors.country" x-text="errors.country"></p>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="declaration1" name="declarations[authorizedSignatory]" type="checkbox" x-model="formData.declarations.authorizedSignatory" 
                            ...
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="declaration2" name="declarations[accurateInformation]" type="checkbox" x-model="formData.declarations.accurateInformation" 
                                        class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                                </div>
                                <div class="ml-3 text-sm">
                            ...
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="declaration3" name="declarations[unEthics]" type="checkbox" x-model="formData.declarations.unEthics" 
                                        class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                                </div>
                                <div class="ml-3 text-sm">
        ...
        <div x-show="showSuccess" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-75" style="display: none;">
            <div class="bg-white p-6 max-w-md w-full rounded-lg shadow-xl transform transition-all sm:max-w-lg">
                <div class="text-center">
                    ...
                    <div class="mt-6">
                        <button @click="window.location.href = '/login'" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            Go to Login Page
                        </button>
                    </div>
                </div>
    ...
    function registrationForm() {
        return {
            ...
            validateField(field) {
                ...
                if (!this.formData[field] && field !== 'terms' && !field.startsWith('declarations')) {
                    return true; // Will be caught by server validation if required
                }
                
                // Send AJAX request to validate the field
                fetch('{{ route("supplier.validate-field") }}', {
                    method: 'POST',
                    headers: {
                ...
                // This is a fallback for when the server is not available
                switch(field) {
                    case 'userEmailConfirm':
                        if (this.formData.userEmail !== this.formData.userEmailConfirm) {
            ...
            validateStep() {
                ...
                if (this.currentStep === 0) {
                    ...
                    fields = ['taxpayerId', 'companyName', 'contactDetails', 'legalAddress', 'country'];
                } 
                    ...
                    fields = ['directorFirstName', 'directorLastName'];
                    ...
                    fields = ['userFirstName', 'userLastName', 'userEmail', 'userEmailConfirm', 'password', 'passwordConfirm', 'emailPreference', 'terms'];
                }
                
                // Validate all fields for this step
                fields.forEach(field => {
                    const fieldValue = field.includes('.') 
                        ? field.split('.').reduce((obj, key) => obj[key], this.formData) 
                        : this.formData[field];
                    
                    if (!fieldValue) {
                        this.errors[field] = `This field is required`;
                        isValid = false;
                    } else if (this.validateField(field) === false) {
                        isValid = false;
                    }
                });
            ...
            submitForm() {
                ...
                this.progressStatus = 'Initializing registration...';
                
                // Create form data
                const form = document.getElementById('registration-form');
                const formData = new FormData(form);
                
                // Add declarations separately as they're nested
                formData.append('declarations[authorizedSignatory]', this.formData.declarations.authorizedSignatory ? '1' : '0');
                formData.append('declarations[accurateInformation]', this.formData.declarations.accurateInformation ? '1' : '0');
                formData.append('declarations[unEthics]', this.formData.declarations.unEthics ? '1' : '0');
                ...
                const progressInterval = setInterval(() => {
                    if (progress < 90) {
                        ...
                        if (progress < 30) {
                            ...
                            this.progressStatus = "Creating your account...";
                        }
                    }
                }, 700);
                    ...
                    setTimeout(() => {
                        ...
                        if (data.success) {
                            ...
                            if (data.errors) {
                                ...
                                // If there are errors for a specific step, go to that step
                                const errorFields = Object.keys(data.errors);
                                
                                if (errorFields.some(field => ['taxpayerId', 'companyName', 'contactDetails', 'legalAddress', 'country'].includes(field))) {
                                    this.currentStep = 0;
                                } else if (errorFields.some(field => ['bankingProof', 'registrationDocuments', 'womenOwnershipDocument'].includes(field))) {
                                    this.currentStep = 1;
                                } else if (errorFields.some(field => ['directorFirstName', 'directorLastName', 'directorPosition', 'directorEmail'].includes(field))) {
                                    this.currentStep = 2;
                                } else {
                                    this.currentStep = 3;
                                }
